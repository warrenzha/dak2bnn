...
class NNSVGPMNIST:
    def __init__(self, args):
        ...
        feature_extractor = CNN(num_classes=args.num_nnsvgp_proj, classifier=True)
        self.model = NNSVGPClassification(
            feature_extractor=feature_extractor,
            num_inducing=args.num_ip_svgp,
            num_classes=10,
            likelihood=gpytorch.likelihoods.SoftmaxLikelihood(num_features=10, num_classes=10),
            num_dim=feature_extractor.out_features,
        ).to(self.device)

        self.optimizer = torch.optim.Adadelta([
            {'params': self.model.feature_extractor.parameters(), 'weight_decay': 1e-4},
...
if __name__ == '__main__':
    ...
    model = NNSVGPClassification(
        feature_extractor=feature_extractor,
        num_inducing=512,
        num_classes=10,
        likelihood=gpytorch.likelihoods.SoftmaxLikelihood(num_features=16, num_classes=10),
        num_dim=16,
    )
