...
from dak.models.svdkl_variational import SVDKLRegression

class SVDKLUCI(object):
        ...
        with gpytorch.settings.num_likelihood_samples(args.num_mc_train):
            for epoch in epochs_iter:    
                    ...
                    for (input, target) in minibatch_iter:
                        ...
                        output = model(input)
                        loss = -mll(output, target.unsqueeze(-1))
                        loss.backward()
                        optimizer.step()
    ...
    def test(self, test_loader, model):
        ...
        with torch.no_grad() and gpytorch.settings.num_likelihood_samples(args.num_mc_test):
            for (input, target) in test_loader:
                ...
                means = preds.mean.squeeze(-1)
                stds = preds.variance.squeeze(-1).sqrt()
